! function() {
	var entityMap = {
		...
		nameChar = new RegExp("[\\-\\.0-9" + nameStartChar.source.slice(1, -1) +
			"\\u00B7\\u0300-\\u036F\\u203F-\\u2040]"),
		tagNamePattern = new RegExp("^" + nameStartChar.source + nameChar.source + "*(?::" + nameStartChar.source +
			nameChar.source + "*)?$"),
		S_TAG = 0,
		S_ATTR = 1,
		S_ATTR_SPACE = 2,
	...
	function parse(e, t, n, r, i) {
		...
		function appendText(t) {
			if (t > d) {
				...
				c && position(d), r.characters(n, 0, t - d), d = t
			}
		}

		function position(t, n) {
			for (; t >= a && (n = s.exec(e));) o = n.index, a = o + n[0].length, c.lineNumber++;
			...
			try {
				...
				if (p < 0) {
					if (!e.substr(d).match(/^\s*$/)) {
						...
						m.appendChild(N), r.currentElement = N
					}
					return
				}
				switch (p > d && appendText(p), e.charAt(p + 1)) {
					case "/":
						var E = e.indexOf(">", p + 3),
							h = e.substring(p + 2, E),
							f = u.pop();
						E < 0 ? (h = e.substring(p + 2).replace(/[\s<].*/, ""), i.error("end tag name: " + h +
							" is not complete:" + f.tagName), E = p + 1 + h.length) : h.match(/\s</) && (h = h
							.replace(/[\s<].*/, ""), i.error("end tag name: " + h + " maybe not complete"), E = p +
							1 + h.length);
						var _ = f.localNSMap,
							T = f.tagName == h;
						if (T || f.tagName && f.tagName.toLowerCase() == h.toLowerCase()) {
							if (r.endElement(f.uri, f.localName, h), _)
								for (var g in _) r.endPrefixMapping(g);
							T || i.fatalError("end tag name: " + h + " is not match the current start tagName:" + f
								.tagName)
						} else u.push(f);
						E++;
						break;
					case "?":
						c && position(p), E = parseInstruction(e, p, r);
						break;
					case "!":
						c && position(p), E = parseDCC(e, p, r, i);
						break;
					default:
						c && position(p);
						var D = new ElementAttributes,
							S = u[u.length - 1].currentNSMap,
							A = (E = parseElementStartPart(e, p, D, S, entityReplacer, i), D.length);
						if (!D.closed && fixSelfClosed(e, E, D.tagName, l) && (D.closed = !0, n.nbsp || i.warning(
								"unclosed xml attribute")), c && A) {
							for (var v = copyLocator(c, {}), C = 0; C < A; C++) {
								var O = D[C];
								position(O.offset), O.locator = copyLocator(c, {})
							}
							r.locator = v, SaxO.appendElement(D, r, S) && u.push(D), r.locator = c
						} else SaxO.appendElement(D, r, S) && u.push(D);
						"" !== D.uri || D.closed ? E++ : E = parseHtmlSpecialContent(e, E, D
							.tagName, entityReplacer, r)
				}
			} catch (e) {
				i.error("element parse error: " + e), E = -1
			}
	...
	function parseElementStartPart(e, t, n, r, i, o) {
		for (var a, s = ++t, c = S_TAG;;) {
			...
			switch (u) {
				case "=":
					if (c === S_ATTR) a = e.slice(t, s), c = S_EQ;
					else {
						if (c !== S_ATTR_SPACE) throw new Error("attribute equal must after attrName");
						c = S_EQ
					}
					break;
				case "'":
				case '"':
					if (c === S_EQ || c === S_ATTR) {
						if (c === S_ATTR && (o.warning('attribute value must after "="'), a = e.slice(t, s)), t = s + 1,
							!((s = e.indexOf(u, t)) > 0)) throw new Error("attribute value no end '" + u + "' match");
						l = e.slice(t, s).replace(/&#?\w+;/g, i), n.add(a, l, t - 1), c = S_ATTR_END
					} else {
						if (c != S_ATTR_NOQUOT_VALUE) throw new Error('attribute value must after "="');
						l = e.slice(t, s).replace(/&#?\w+;/g, i), n.add(a, l, t), o.warning('attribute "' + a +
							'" missed start quot(' + u + ")!!"), t = s + 1, c = S_ATTR_END
					}
					break;
				case "/":
					switch (c) {
					...
					break;
				case "":
					return o.error("unexpected end of input"), c == S_TAG && n.setTagName(e.slice(t, s)), s;
				case ">":
					switch (c) {
						case S_TAG:
							n.setTagName(e.slice(t, s));
						...
						case S_ATTR_SPACE:
							c === S_ATTR_SPACE && (l = a), c == S_ATTR_NOQUOT_VALUE ? (o.warning('attribute "' + l +
								'" missed quot(")!!'), n.add(a, l.replace(/&#?\w+;/g, i), t)) : (
								"" === r[""] && l.match(
									/^(?:disabled|checked|selected)$/i) || o.warning('attribute "' + l +
									'" missed value!! "' + l + '" instead!!'), n.add(l, l, t));
							break;
						case S_EQ:
				...
				default:
					if (u <= " ") switch (c) {
						case S_TAG:
							n.setTagName(e.slice(t, s)), c = S_TAG_SPACE;
							break;
						case S_ATTR:
							a = e.slice(t, s), c = S_ATTR_SPACE;
							break;
						case S_ATTR_NOQUOT_VALUE:
							var l = e.slice(t, s).replace(/&#?\w+;/g, i);
							o.warning('attribute "' + l + '" missed quot(")!!'), n.add(a, l, t);
						case S_ATTR_END:
							c = S_TAG_SPACE
					} else switch (c) {
						case S_ATTR_SPACE:
							n.tagName;
							"" === r[""] && a.match(
									/^(?:disabled|checked|selected)$/i) || o.warning('attribute "' + a +
									'" missed value!! "' + a + '" instead2!!'), n.add(a, a, t), t = s, c =
								S_ATTR;
							break;
						case S_ATTR_END:
	...
	function parseHtmlSpecialContent(e, t, n, r, i) {
		if (/^(?:script|textarea)$/i.test(n)) {
			var o = e.indexOf("</" + n + ">", t),
				a = e.substring(t + 1, o);
			if (/[&<]/.test(a)) return /^script$/i.test(n) ? (i.characters(a, 0, a.length), o) : (a = a.replace(
				/&#?\w+;/g, r), i.characters(a, 0, a.length), o)
		}
		return t + 1
	}
			...
			default:
				...
				if (a > 1 && /!doctype/i.test(o[0][0])) {
					var s = o[1][0],
						...
						u = a > 4 && o[4][0],
						l = o[a - 1];
					return n.startDTD(s, c && c.replace(/^(['"])(.*?)\1$/, "$2"), u && u.replace(/^(['"])(.*?)\1$/,
						"$2")), n.endDTD(), l.index + l[0].length
				}
		}
		return -1
	}
	...
	SaxO = {}, XMLReader.prototype = {
		parse: function(e, t, n) {
			...
			r.startDocument(), _copy(t, t = {}), parse(e, t, n, r, this.errorHandler), r.endDocument()
		}
		...
		for (var r = e.tagName, i = null, o = e.length; o--;) {
			var a = e[o],
				s = a.qName,
				c = a.value;
			if ((p = s.indexOf(":")) > 0) var u = a.prefix = s.slice(0, p),
				l = s.slice(p + 1),
				d = "xmlns" === u && l;
			else l = s, u = null, d = "xmlns" === s && "";
			a.localName = l, !1 !== d && (null == i && (i = {}, _copy(n, n = {})), n[d] = i[d] = c, a.uri =
				"", t.startPrefixMapping(d, c))
		}
		for (o = e.length; o--;) {
			(u = (a = e[o]).prefix) && ("xml" === u && (a.uri = ""), "xmlns" !==
				u && (a.uri = n[u || ""]))
		}
		var p;
		(p = r.indexOf(":")) > 0 ? (u = e.prefix = r.slice(0, p), l = e.localName = r.slice(p + 1)) : (u = null, l =
			e.localName = r);
		var m = e.uri = n[u || ""];
		if (t.startElement(m, l, r, e), !e.closed) return e.currentNSMap = n, e.localNSMap = i, !0;
		if (t.endElement(m, l, r), i)
			for (u in i) t.endPrefixMapping(u)
	...
	var htmlns = "",
		...
		NO_MODIFICATION_ALLOWED_ERR = ExceptionCode.NO_MODIFICATION_ALLOWED_ERR = (ExceptionMessage[7] =
		...
		INVALID_MODIFICATION_ERR = ExceptionCode.INVALID_MODIFICATION_ERR = (ExceptionMessage[13] =
			"Invalid modification", 13),
	...
	function DOMException(e, t) {
		...
		else n = this, Error.call(this, ExceptionMessage[e]), this.message = ExceptionMessage[e], Error
			.captureStackTrace && Error.captureStackTrace(this, DOMException);
	...
	function _onAddAttribute(e, t, n) {
		e && e._inc++, "" == n.namespaceURI && (t._nsMap[n.prefix ? n.localName : ""] = n
			.value)
	}

	function _onRemoveAttribute(e, t, n, r) {
		e && e._inc++, "" == n.namespaceURI && delete t._nsMap[n.prefix ? n.localName : ""]
	}
	...
	function _removeChild(e, t) {
		var n = t.previousSibling,
			r = t.nextSibling;
		return n ? n.nextSibling = r : e.firstChild = r, r ? r.previousSibling = n : e.lastChild = n, _onUpdateChild(e
			.ownerDocument, e), t
	}
	...
	function _insertBefore(e, t, n) {
		...
		i.previousSibling = a, o.nextSibling = n, a ? a.nextSibling = i : e.firstChild = i, null == n ? e.lastChild =
			o : n.previousSibling = o;
		do {
			i.parentNode = e
		} while (i !== o && (i = i.nextSibling));
		return _onUpdateChild(e.ownerDocument || e, e), t.nodeType == DOCUMENT_FRAGMENT_NODE && (t.firstChild = t
			.lastChild = null), t
	}
		...
		switch (e.nodeType) {
			case ELEMENT_NODE:
				...
				var o = e.attributes,
					...
					s = e.firstChild,
					c = e.tagName;
				n = htmlns === e.namespaceURI || n, t.push("<", c);
				for (var u = 0; u < a; u++) {
				...
				if (needNamespaceDefine(e, n, i)) {
					d = e.prefix || "", p = e.namespaceURI, m = d ? " xmlns:" + d : " xmlns";
					t.push(m, '="', p, '"'), i.push({
						prefix: d,
						namespace: p
					})
				...
				if (s || n && !/^(?:meta|link|img|br|hr|input)$/i.test(c)) {
					if (t.push(">"), n && /^script$/i.test(c))
						for (; s;) s.data ? t.push(s.data) : serializeToString(s, t, n, r, i), s = s.nextSibling;
					else
						for (; s;) serializeToString(s, t, n, r, i), s = s.nextSibling;
					t.push("</", c, ">")
				} else t.push("/>");
				return;
			case DOCUMENT_NODE:
			...
			case DOCUMENT_TYPE_NODE:
				var N = e.publicId,
					E = e.systemId;
				if (t.push("<!DOCTYPE ", e.name), N) t.push(' PUBLIC "', N), E && "." != E && t.push('" "', E), t.push(
					'">');
	...
	function importNode(e, t, n) {
		...
		if (r || (r = t.cloneNode(!1)), r.ownerDocument = e, r.parentNode = null, n)
			for (var i = t.firstChild; i;) r.appendChild(importNode(e, i, n)), i = i.nextSibling;
		return r
	}
		...
		switch (t.childNodes && (r.childNodes = new NodeList), r.ownerDocument = e, r.nodeType) {
			case ELEMENT_NODE:
				var a = t.attributes,
					...
					c = a.length;
				s._ownerElement = r;
				for (var u = 0; u < c; u++) r.setAttributeNode(cloneNode(e, a.item(u), !0));
				break;
			case ATTRIBUTE_NODE:
				n = !0
		}
		if (n)
			for (var l = t.firstChild; l;) r.appendChild(cloneNode(e, l, n)), l = l.nextSibling;
		return r
	}
	...
	DOMException.prototype = Error.prototype, copy(ExceptionCode, DOMException), NodeList.prototype = {
			...
			setNamedItemNS: function(e) {
				...
				return t = this.getNamedItemNS(e.namespaceURI, e.localName), _addNamedNode(this._ownerElement, this,
					e, t), t
			},
			...
			createDocument: function(e, t, n) {
				var r = new Document;
				if (r.implementation = this, r.childNodes = new NodeList, r.doctype = n, n && r.appendChild(n), t) {
			...
			normalize: function() {
				for (var e = this.firstChild; e;) {
					var t = e.nextSibling;
					t && t.nodeType == TEXT_NODE && e.nodeType == TEXT_NODE ? (this.removeChild(t), e.appendData(t
						.data)) : (e.normalize(), e = t)
				}
			},
			...
			createElement: function(e) {
				var t = new Element;
				return t.ownerDocument = this, t.nodeName = e, t.tagName = e, t.childNodes = new NodeList, (t
					.attributes = new NamedNodeMap)._ownerElement = t, t
			},
			...
			createElementNS: function(e, t) {
				...
				return n.childNodes = new NodeList, n.ownerDocument = this, n.nodeName = t, n.tagName = t, n
					.namespaceURI = e, 2 == r.length ? (n.prefix = r[0], n.localName = r[1]) : n.localName = t, i
					._ownerElement = n, n
			},
			...
			appendChild: function(e) {
				return e.nodeType === DOCUMENT_FRAGMENT_NODE ? this.insertBefore(e, null) : _appendSingleChild(this,
					e)
			},
			...
			getElementsByTagName: function(e) {
				return new LiveNodeList(this, function(t) {
					...
					return _visitNode(t, function(r) {
						r === t || r.nodeType != ELEMENT_NODE || "*" !== e && r.tagName != e || n
							.push(r)
					}), n
				})
			},
			getElementsByTagNameNS: function(e, t) {
				return new LiveNodeList(this, function(n) {
					...
					return _visitNode(n, function(i) {
						i === n || i.nodeType !== ELEMENT_NODE || "*" !== e && i.namespaceURI !==
							e || "*" !== t && i.localName != t || r.push(i)
					}), r
				})
			}
		}, Document.prototype.getElementsByTagName = Element.prototype.getElementsByTagName, Document.prototype
		.getElementsByTagNameNS = Element.prototype.getElementsByTagNameNS, _extends(Element, Node), Attr.prototype
		.nodeType = ATTRIBUTE_NODE, _extends(Attr, Node), CharacterData.prototype = {
			data: "",
			...
			replaceData: function(e, t, n) {
				n = this.data.substring(0, e) + n + this.data.substring(e + t), this.nodeValue = this.data = n, this
					.length = n.length
			}
		...
		}, _extends(CDATASection, CharacterData), DocumentType.prototype.nodeType = DOCUMENT_TYPE_NODE, _extends(
			DocumentType, Node), Notation.prototype.nodeType = NOTATION_NODE, _extends(Notation, Node), Entity.prototype
		.nodeType = ENTITY_NODE, _extends(Entity, Node), EntityReference.prototype.nodeType = ENTITY_REFERENCE_NODE,
		_extends(EntityReference, Node), DocumentFragment.prototype.nodeName = "#document-fragment", DocumentFragment
		.prototype.nodeType = DOCUMENT_FRAGMENT_NODE, _extends(DocumentFragment, Node), ProcessingInstruction.prototype
		.nodeType = PROCESSING_INSTRUCTION_NODE, _extends(ProcessingInstruction, Node), XMLSerializer.prototype
		.serializeToString = function(e, t, n) {
	...
	function buildErrorHandler(e, t, n) {
		...
		return n = n || {}, build("warning"), build("error"), build("fatalError"), r
	}
	...
	"function" == typeof require && (exports.DOMImplementation = DOMImplementation, exports.XMLSerializer =
			XMLSerializer), DOMParser.prototype.parseFromString = function(e, t) {
			var n = this.options,
				r = new XMLReader,
			...
			return a && i.setDocumentLocator(a), r.errorHandler = buildErrorHandler(o, i, a), r.domBuilder = n
				.domBuilder || i, c && (s[""] = ""), s.xml = s.xml ||
				"", e ? r.parse(e, s, u) : r.errorHandler.error(
					"invalid doc source"), i.doc
		}, DOMHandler.prototype = {
			startDocument: function() {
				this.doc = (new DOMImplementation).createDocument(null, null, null), this.locator && (this.doc
			...
			startElement: function(e, t, n, r) {
				...
				for (var s = 0; s < a; s++) {
					e = r.getURI(s);
					var c = r.getValue(s),
						u = (n = r.getQName(s), i.createAttributeNS(e, n));
					this.locator && position(r.getLocator(s), u), u.value = u.nodeValue = c, o.setAttributeNode(u)
				}
			},
			...
			characters: function(e, t, n) {
				if (e = _toString.apply(this, arguments)) {
					...
					this.currentElement ? this.currentElement.appendChild(r) : /^\s*$/.test(e) && this.doc
						.appendChild(r), this.locator && position(this.locator, r)
				}
			},

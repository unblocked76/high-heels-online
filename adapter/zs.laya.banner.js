window.zs = window.zs || {}, window.zs.laya = window.zs.laya || {}, function(e, t) {
    ...
    class n {
        ...
        updateBanner(e, t, i, n) {
            ...
            this.hideAll(), this.lastFreshTime = new Date().getTime(), this.isWait = e, this.left = t, 
            this.bottom = i, this.length = n ? Math.min(this.bannerIds.length, n) : 1;
            var s = 0;
            let a = 0;
            for (let e = this.wxbannerArray.length - 1; e >= 0; e--) {
                let t = this.wxbannerArray[e];
                if (this.wxbannerArray.length > 5 && t.showEd && t.destroy(), t.bannerAd) {
                    if (t.isLoad || a++, t.isLoad && !t.showEd && !t.isShow && (console.log("存在加载完成但未展示的banner"), 
                    this.isWait || t.show(), ++s >= this.length)) return;
                } else console.log("banner已销毁"), this.wxbannerArray.splice(e, 1);
            }
            if (a >= this.length || this.inErrTime) return console.log("当前拉取中大于配置次或者出现banner报错,暂停拉取"), 
            void (this.isWait || this.showBanner(this.left, this.bottom, this.length - s));
            this.createNum = this.length - s, this.createBanner();
        }
        createBanner() {
